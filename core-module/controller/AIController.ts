...
export default class AIController {
  public static async injestFile(req: Request, res: Response) {
    try {
      ...
      logger.info({
        message: `[AIController.injestFile] create collection workspaceId ${workspaceId}, channelId ${channelId}`,
      });
      await AIService.createCollectionIfNotExist(collectionName);

      logger.info({
        message: `[AIController.injestFile] invoking fileToVector workspaceId ${workspaceId}, channelId ${channelId}`,
      });
  ...
  public static async getInjestFile(req: Request, res: Response) {
    ...
    try {
      const response = await AIController.getInjestFileHelper(
        question,
        history,
        workspaceId,
        channelId,
  ...
  public static async getInjestFileHelper(
    question: string,
    history: string[],
    workspaceId: string,
    channelId: string,
    ...
    try {
      logger.info({
        message: `[AIController.getInjestFile] gettingInjestedFile, question:${question}, workspaceId:${workspaceId}, channelId:${channelId}`,
      });
      const collectionName = getCollectionName(workspaceId, channelId);
      const sanitizedQuestion = question.trim().replace(/\n/g, " ");

      logger.info({
        message: `[AIController.getInjestFile] invoking Chroma.fromExistingCollection, question:${question}, workspaceId:${workspaceId}, channelId:${channelId}`,
      });
      ...
      logger.info({
        message: `[AIController.getInjestFile] fetching custom prompt`,
      });
      const { customPrompt, enableHumanHandover } = await getCustomPrompt(
        channelId
      );

      logger.info({
        message: `[AIController.getInjestFile] makechain, question:${question}, workspaceId:${workspaceId}, channelId:${channelId}`,
      });
      ...
      logger.info({
        message: `[AIController.getInjestFile] chain.call, question:${question}, workspaceId:${workspaceId}, channelId:${channelId}`,
      });
      const response = await chain.call({
        question: sanitizedQuestion,
        chat_history: history || [],
      });
      await AIQuestionLogModel.create({
        aiQuestionLogId,
        question,
        workspaceId,
        channelId,
  ...
  public static async downloadFile(req: Request, res: Response) {
    try {
      ...
      const file = await KnowledgeBaseModel.findOne({
        fileId,
        channelId,
        workspaceId,
      });
    ...
    try {
      ...
      const uploadedUrls = await Promise.all(
        ((req.files || []) as any[])?.map(async (file: any) => {
          ...
          logger.info({
            message: `[AIController.uploadFiles] uploading file ${fileName}`,
          });
          ...
          logger.info({
            message: `[AIController.uploadFiles] file uploaded Url: ${fileUrl}`,
          });
          return {
            url: fileUrl,
  ...
  public static async getAnalytics(req: Request, res: Response) {
    try {
      ...
      const getFileUploadedCount = await KnowledgeBaseModel.countDocuments({
        workspaceId,
      }).exec();

      const getAICalls = await AIQuestionLogModel.countDocuments({
        workspaceId,
      }).exec();
