...
app.use(
  cors({
    origin: [
      ...
      "https://zeon-finance.flutterflow.app",
      "http://localhost:5687",
      "https://app.zeonhq.com",
      "http://zeon-dashboard:5687",
    ],
    // ALLOW ALL METHODs
    methods: "*",
    // ALLOW ALL HEADERS
...
app.use("/integrations", verifyIdentity, integrationRoutes);
app.use("/agent", verifyIdentity, aiAgentRoutes);
app.use("/ai", AIRoute);

app.use("/incoming/:agentId", AgentController.incomingCallHandler);

app.post(
  "/internal/communication/send-email",
  CommunicationController.sendEmail
);

app.post("/internal/slack/message", oauthController.sendMessage);

app.ws("/incoming-call/:agentId", function (ws, req) {
  const { agentId } = req.params;

  const aIAgentService = new AIAgentService(agentId);
  aIAgentService.handleIncomingCall(ws);
});
  ...
  async (request, response) => {
    ...
    switch (event.type) {
      case "checkout.session.completed":
        ...
        newSubscription.subscriptionEndTime =
          thisTime + 30 * 24 * 60 * 60 * 1000;
      ...
      case "customer.subscription.deleted":
        ...
        const priceDeleted = await stripe.prices.retrieve(
          subscriptionDeleted.items.data[0].price.id
        );
        const lookupDeleted = priceDeleted.lookup_key;
        const customerDeleted = subscriptionDeleted.customer;
        const workspaceDeleted = await Workspace.findOne({
        ...
        subscriptionDeleted.subscriptionEndTime =
          currentTimeDeleted + 30 * 24 * 60 * 60 * 1000;
      ...
      case "customer.subscription.paused":
        ...
        const pricePaused = await stripe.prices.retrieve(
          subscriptionPaused.items.data[0].price.id
        );
        const lookupPaused = pricePaused.lookup_key;
        const customerPaused = subscriptionPaused.customer;
        const workspacePaused = await Workspace.findOne({
          stripeCustomerId: customerPaused,
        });
        if (!workspacePaused) {
          return response.status(400).json({ message: "Workspace not found" });
        }
        ...
        subscriptionPaused.subscriptionEndTime =
          currentTimePaused + 30 * 24 * 60 * 60 * 1000;
...
app.listen(port, () => {
  console.log(
    `Server running at port ${port}, Node version: ${process.version}`
  );
});
