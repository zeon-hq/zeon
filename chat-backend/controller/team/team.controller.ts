...
export const createTeam = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // create a workspace
    const workspace = await createWorkspace({ workspaceName, primaryContactEmail: user.email, primaryContactName: user.name, signupDetails: { signupMode: "api", isVerified: true }, modules })
    // create a entry in user table with userId and workspaceId
    await createUserWithUserIdAndWorkspaceId({ userId: user.userId, workspaceId: workspace.workspaceId, role: "owner" })
...
export const getTeamData = async (req: SessionRequest, res: Response) => {
  ...
  try {
    ...
    let users = allUsers.filter((user: any) => user.roleId !== "owner" && user.roleId !== "admin")
      ...
      const  req = https.request(options, (apiRes) => {
        apiRes.on('data', (chunk) => {
          ...
          return res.status(200).json({
            ...
            uploadedUrl: `https://zeonhq.b-cdn.net/${FILENAME_TO_UPLOAD}` || undefined,
          });
