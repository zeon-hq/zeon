    ...
    try {
        ...
        if (data.childCategories) {
            ...
            for (let i = 0; i < data.childCategories.length; i++) {
                ...
                if(!childCategory.name) return res.status(400).json({ message: 'Child category name is required' });
...
export const getCategoryById = async (req: Request, res: Response) => {
    try {
        ...
        const category = await Category.findOne({ categoryId: categoryId, isDeleted: false });
        if (!category) {
            return res.status(404).json({ message: 'Category not found' });
        }
        return res.status(200).json(category);
    } catch (error) {
        console.error(`Error getting category by ID: ${req.params.categoryId}`, error);
        return res.status(500).json({ message: error.message || error || 'Internal server error' });
    }
}
